...
from natrium.applications.yggdrasil import exceptions, user_auth_cooldown_bucket, auth_token_pool
import bcrypt
...
async def authenticate(request: Request):
    ...
    with orm.db_session:
        ...
        if not user_auth_cooldown_bucket.get(user.Email):
            user_auth_cooldown_bucket.setByTimedelta(user.Email, "LOCKED")
        else:
            return error_handle(exceptions.InvalidCredentials())
        
        AuthentidcateVerifyResult = bcrypt.hashpw(data.get("password").encode(), user.Salt) == user.Password
